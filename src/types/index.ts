...
export type VPNStatus = 'disconnected' | 'connecting' | 'handshaking' | 'connected' | 'error';
...
export interface VPNModuleInterface {
    ...
    // Events
    addListener(event: 'statusChanged' | 'error', callback: (data: any) => void): void;
...
export function isVPNConnectionStats(value: unknown): value is VPNConnectionStats {
...
export function isVPNState(value: unknown): value is VPNState {
    ...
    return (
        ...
        (state.activeProfileId === undefined || typeof state.activeProfileId === 'string') &&
        (state.error === undefined || typeof state.error === 'string') &&
        (state.connectedAt === undefined || state.connectedAt instanceof Date || typeof state.connectedAt === 'string') &&
...
export function sanitizeProfileInput(input: Partial<ProxyProfile>): Partial<ProxyProfile> {
    return {
        ...input,
