...
export class StorageService {
    ...
    async getProfiles(): Promise<ProxyProfile[]> {
        try {
            // First, sync from native module (in case profiles were added via ADB)
    ...
    private async syncFromNative(): Promise<void> {
        try {
            ...
            const storedData = await AsyncStorage.getItem(StorageService.PROFILES_KEY);
            const currentProfiles: StoredProfile[] = storedData ? JSON.parse(storedData) : [];
    ...
    async clearAll(): Promise<void> {
        try {
            ...
            await Promise.all(
                profiles.map(p => this.deleteCredentials(p.id))
            );
