...
"
  ...
  # Get app name (with .app extension)
  APP_BUNDLE=$(basename "$APP_PATH")
  # Get binary name (without .app extension)
  BINARY_NAME="${APP_BUNDLE%.app}"
  
  echo -e "${GREEN}✓ App bundle: $APP_BUNDLE${NC}"
  echo -e "${GREEN}✓ Binary name: $BINARY_NAME${NC}"
  
  # Save paths
  MAIN_ENTITLEMENTS="$SCRIPT_DIR/ios/CBVVPN/CBVVPN.entitlements"
  EXT_ENTITLEMENTS="$SCRIPT_DIR/ios/CBVVPNProxyExtension/CBVVPNProxyExtension.entitlements"

  # Sign with ldid for TrollStore
  echo -e "${YELLOW}Signing with ldid for TrollStore...${NC}"
  
  # Check if ldid is available
  if ! command -v ldid &> /dev/null; then
    echo -e "${RED}✗ ldid not found!${NC}"
    echo -e "${YELLOW}Installing ldid via Homebrew...${NC}"
    if command -v brew &> /dev/null; then
      brew install ldid
    else
      echo -e "${RED}Homebrew not found. Please install ldid manually:${NC}"
      echo -e "${YELLOW}brew install ldid${NC}"
      exit 1
    fi
  fi
  
  # Sign main app with entitlements
  MAIN_BINARY="$PWD/build/output/Payload/$APP_BUNDLE/$BINARY_NAME"
  if [ -f "$MAIN_ENTITLEMENTS" ]; then
    echo -e "${YELLOW}Signing main app: $MAIN_BINARY${NC}"
    echo -e "${YELLOW}Using entitlements: $MAIN_ENTITLEMENTS${NC}"
    
    if [ ! -f "$MAIN_BINARY" ]; then
      echo -e "${RED}✗ Binary not found: $MAIN_BINARY${NC}"
      exit 1
    fi
    
    ldid "-S$MAIN_ENTITLEMENTS" "$MAIN_BINARY"
    echo -e "${GREEN}✓ Main app signed${NC}"
    
    # Verify signing
    echo -e "${YELLOW}Verifying main app signature...${NC}"
    ldid -e "$MAIN_BINARY" > /tmp/main-ent-check.xml
    if [ -s /tmp/main-ent-check.xml ]; then
      echo -e "${GREEN}✓ Main app entitlements verified:${NC}"
      cat /tmp/main-ent-check.xml | head -15
      rm /tmp/main-ent-check.xml
    else
      echo -e "${RED}✗ Main app entitlements NOT found!${NC}"
    fi
  else
    echo -e "${RED}✗ Main app entitlements not found at: $MAIN_ENTITLEMENTS${NC}"
    exit 1
  fi
  echo ""
  
  # Sign extensions
  if [ -d "$PWD/build/output/Payload/$APP_BUNDLE/PlugIns" ]; then
    for EXTENSION in "$PWD/build/output/Payload/$APP_BUNDLE/PlugIns"/*.appex; do
      if [ -d "$EXTENSION" ]; then
        EXT_NAME=$(basename "$EXTENSION" .appex)
        EXT_BINARY="$EXTENSION/$EXT_NAME"
        
        echo -e "${YELLOW}Signing extension: $EXT_NAME${NC}"
        echo -e "${YELLOW}Binary: $EXT_BINARY${NC}"
        
        if [ ! -f "$EXT_BINARY" ]; then
          echo -e "${RED}✗ Extension binary not found: $EXT_BINARY${NC}"
          continue
        fi
        
        if [ -f "$EXT_ENTITLEMENTS" ]; then
          ldid "-S$EXT_ENTITLEMENTS" "$EXT_BINARY"
          echo -e "${GREEN}✓ Extension signed: $EXT_NAME${NC}"
          
          # Verify signing
          echo -e "${YELLOW}Verifying extension signature...${NC}"
          ldid -e "$EXT_BINARY" > /tmp/ext-ent-check.xml
          if [ -s /tmp/ext-ent-check.xml ]; then
            echo -e "${GREEN}✓ Extension entitlements verified${NC}"
            rm /tmp/ext-ent-check.xml
          else
            echo -e "${RED}✗ Extension entitlements NOT found!${NC}"
          fi
        else
          echo -e "${RED}✗ Extension entitlements not found at: $EXT_ENTITLEMENTS${NC}"
        fi
      fi
    done
  fi
  ...
  # Remove code signatures (keep ldid signatures)
  echo -e "${YELLOW}Removing Apple code signatures...${NC}"
  find "$PWD/build/output/Payload/$APP_BUNDLE" -name "_CodeSignature" -type d -prune -exec rm -rf {} + 2>/dev/null || true
  find "$PWD/build/output/Payload/$APP_BUNDLE" -name "embedded.mobileprovision" -type f -exec rm -f {} + 2>/dev/null || true
  find "$PWD/build/output/Payload/$APP_BUNDLE" -name "embedded.entitlements" -type f -exec rm -f {} + 2>/dev/null || true
  echo -e "${GREEN}✓ Apple signatures removed (ldid signatures preserved)${NC}"
